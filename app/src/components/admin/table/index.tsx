...
type SortableField = "name" | "description" | "contact";
const Table = ({ button, initialVenues }: tableProps) => {
  ...
  const handleSort = (field: "name" | "description" | "contact") => {
    if (sortBy === field) {
  ...
  const sortedVenues = sortBy
    ? sort(filteredVenues)[sortOrder === "asc" ? "asc" : "desc"](
        (venue) => venue[sortBy]
      )
    : filteredVenues;
  ...
  return (
    <div className="mx-auto p-2">
      ...
      <div className="flex justify-end mb-4 gap-2">
        ...
        {button && (
          <button className="border-2 border-red-400 p-1 text-center rounded-md hover:bg-red-300 transition-all duration-300 flex flex-row-reverse justify-center items-center gap-2">
            {button}
            <Image
              alt="logout"
              src="/icons/plus.svg"
